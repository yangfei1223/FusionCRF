...
class CONV_2D(nn.Module):
    def __init__(self,
        ...
        self.conv = nn.Conv2d(in_channels, out_channels,
                              ...
                              bias=bias)
        self.bn = nn.BatchNorm2d(out_channels, momentum=0.02) if bn else None
        self.activation = activation
...
class Base(nn.Module):
    ...
    def get_pred(self):
        return self.y_hat.max(dim=1)[1].cpu().numpy()

    def get_prob(self):
        return self.y_hat.softmax(dim=1)[:,1, ...].squeeze().cpu().numpy()

    def get_label(self):
        return self.y.cpu().numpy()
