...
def eval_road(li_pred, li_gt):
    ...
    for property in li_property:
        ...
        # print('{}: {:.2f}'.format(property, eval_dict[property] * 100))

    print('Finished evaluating!')
    return eval_dict['MaxF']

if __name__ == '__main__':
    gt_path = '/media/yangfei/Repository/KITTI/data_road/training/gt_2'
    pred_path = '/home/yangfei/myPaper/FusionCRF/RUNS/results/DenseCRFFusion'
    filelist = os.listdir(pred_path)
    li_pred = []
    li_gt = []
    for name in filelist:
        print(name)
        filename = os.path.join(gt_path, name)
        label_true = cv2.imread(filename, cv2.IMREAD_UNCHANGED)
        filename = os.path.join(pred_path, name)
        label_pred = cv2.imread(filename, cv2.IMREAD_UNCHANGED)
        # label_pred = (label_pred > 127).astype(np.uint8)
